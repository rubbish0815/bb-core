...
function setTempKeys(temp_priv_key, prev_temp_priv_key, fnSaveTempKeys){
	logger.debug("setTempKeys", temp_priv_key, prev_temp_priv_key);
	objMyTempDeviceKey = {
...
function setDeviceName(device_name){
	logger.log("setDeviceName", device_name);
	my_device_name = device_name;
}

function setDeviceHub(device_hub){
	logger.log("setDeviceHub", device_hub);
...
function loginToHub(){
	if (!objMyPermanentDeviceKey)
		return logger.debug("objMyPermanentDeviceKey not set yet, can't log in");
	if (!objMyTempDeviceKey)
		return logger.debug("objMyTempDeviceKey not set yet, can't log in");
	if (!my_device_hub)
		return logger.debug("my_device_hub not set yet, can't log in");
	logger.log("logging in to hub "+my_device_hub);
...
function genPrivKey(){
	var privKey;
	do {
		logger.debug("generating new priv key");
...
function rotateTempDeviceKeyIfCouldBeAlreadyUsed(){
	...
	if (actual_interval > TEMP_DEVICE_KEY_ROTATION_PERIOD + 1000)
		return logger.debug("woke up after sleep or high load, will skip rotation");
	if (objMyTempDeviceKey.use_count === 0) // new key that was never used yet
		return logger.debug("the current temp key was not used yet, will not rotate");
...
function rotateTempDeviceKey(){
	if (!saveTempKeys)
		return logger.debug("no saving function");
	logger.log("will rotate temp device key");
	network.findOutboundPeerOrConnect(conf.WS_PROTOCOL+my_device_hub, function onLocatedHubForRotation(err, ws){
		if (err)
			return logger.debug('will not rotate because: '+err);
		if (ws.readyState !== ws.OPEN)
			return logger.debug('will not rotate because connection is not open');
		if (!ws.bLoggedIn)
			return logger.debug('will not rotate because not logged in'); // reconnected and not logged in yet
		...
		saveTempKeys(new_priv_key, objMyTempDeviceKey.priv, function(err){
			if (err){
				logger.error('failed to save new temp keys, canceling: '+err);
				return;
			}
...
function decryptPackage(objEncryptedPackage){
	...
	if (objEncryptedPackage.dh.recipient_ephemeral_pubkey === objMyTempDeviceKey.pub_b64){
		...
		logger.debug("message encrypted to temp key");
	}
		...
		logger.debug("message encrypted to prev temp key");
		...
		logger.debug("message encrypted to permanent key");
	}
	else{
		logger.debug("message encrypted to unknown key");
		setTimeout(function(){
...
function resendStalledMessages(delay){
	var delay = delay || 0;
	logger.debug("resending stalled messages delayed by "+delay+" minute");
	if (!objMyPermanentDeviceKey)
		return logger.debug("objMyPermanentDeviceKey not set yet, can't resend stalled messages");
	mutex.lockOrSkip(['stalled'], function(unlock){
		...
		db.query(
			...
			function(rows){
				logger.debug(rows.length+" stalled messages");
				async.eachSeries(
					rows, 
					function(row, cb){
						...
						var send = function(message){
							...
							//    throw "to mismatch";
							logger.debug('sending stalled '+row.message_hash);
...
function reliablySendPreparedMessageToHub(ws, recipient_device_pubkey, json, callbacks, conn){
	...
	logger.debug('will encrypt and send to '+recipient_device_address+': '+JSON.stringify(json));
...
//	console.log("will encrypt and send: "+text);
	...
	logger.debug(shared_secret.length);
...
function addUnconfirmedCorrespondent(device_pubkey, device_hub, device_name, onDone){
	logger.debug("addUnconfirmedCorrespondent");
...
function getWitnessesFromHub(cb){
	logger.debug('getWitnessesFromHub');
	if (!my_device_hub){
		logger.debug('getWitnessesFromHub: no hub yet');
